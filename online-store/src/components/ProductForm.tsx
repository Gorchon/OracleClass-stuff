...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const handleAddProduct = useCallback(async () => {
    try {
      ...
      const result: Product = { ...formData, id: docRef.id } as any;
      setImagePreview("");
      setFormData(initialFormState);
      onSubmit(result);
    } catch (error) {
  ...
  const handleUpdateProduct = useCallback(async () => {
    ...
    try {
      ...
      const result: Product = { ...formData, id: product.id };
      onSubmit(result);
    } catch (error) {
      console.error("Error updating product:", error);
      setErrors({ submit: "Failed to update product" });
    }
  }, [formData, onSubmit, product]);
    ...
    <>
      <form ref={formRef} onSubmit={handleSubmit} className="product-form">
        ...
        <div className="form-actions">
          ...
          <button
            type="submit"
            className={`submit-button ${product ? "edit-button" : ""}`}
            disabled={isSubmitting}
          >
            {isSubmitting ? (
